...
const SubcategoryTable = ({
      ...
      render: (value, row) => (
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 line-clamp-2 text-xs text-[var(--table-subtext)]">
  ...
  const actions = (subcategory) => (
    <div className="flex items-center justify-end gap-1">
      ...
      <button
        type="button"
        title={subcategory.status === 'Active' ? 'Mark inactive' : 'Mark active'}
        onClick={() => onToggleStatus(subcategory.id)}
        className="rounded-lg p-2 text-[var(--color-gray-400)] transition-standard hover:bg-[var(--surface-muted)] hover:text-[var(--color-primary)]"
      >
        <span className="inline-flex min-w-[54px] items-center justify-center rounded-full bg-[var(--surface-muted)] px-2 py-1 text-[11px] font-semibold text-[var(--color-gray-600)]">
    ...
    <CustomTable
      ...
      toolbarActions={
        <select
          ...
          className="h-11 min-w-[200px] rounded-xl border border-[var(--color-gray-200)] bg-white px-4 text-sm font-semibold text-[var(--table-text)] outline-none"
        >
